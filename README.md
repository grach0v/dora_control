...
| `sync` | reusable aggregator: collect N event-driven inputs, emit one concatenated bundle the moment every input has a fresh sample (no tick), `log.warning` on component-timestamp skew. Bundles the per-arm hardware nodes (no built-in dora join) | — | ❌ |
