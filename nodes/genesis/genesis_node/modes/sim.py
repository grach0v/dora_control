"""`sim` mode — Genesis physics + on-board cameras, driven by per-part joint targets.

Consumes `<part>_joint_target` for every part; on each `tick` applies them, steps physics,
and publishes the whole-robot `state` bundle (per the descriptor `state_layout`) + per-part
`<part>_tcp_pose` + cameras. Genesis runs inline on the dora loop (its renderer needs the main
thread). Genesis does NO IK — pinocchio owns it.
...
class SimMode:
    def __init__(self, cfg: GenesisConfig, node: Node, world=None):
        self.cfg = cfg
        self.node = node
        self.desc = load_descriptor(cfg.scene)
        self.world = world if world is not None else GenesisWorld(cfg, self.desc)
    ...
    def start(self) -> None:
        ...
        logger.info("genesis: ready (%s)", ", ".join(self.parts))
        self._emit_status("ready")

    def _substeps(self) -> int:
        """Physics steps to advance this tick = real time elapsed since the last advance /
        the physics timestep — so the sim tracks wall-clock (~realtime) at any tick rate,
        with no manual `steps_per_tick`. Capped (max_substeps) so a slow machine degrades
        to slower-than-realtime rather than spiralling trying to catch up."""
        now = time.monotonic()
        n = round((now - self._last_wall) / self.world.timestep)
        self._last_wall = now
